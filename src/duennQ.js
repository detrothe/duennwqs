...
class TNode {
    y = 1.0;
    z = 1.0;
    yh = 1.0; // entspricht Koordinate in Hauptrichtung 1 (eta), Ursprung liegt im Schwerpunkt
    zh = 1.0; // entspricht Koordinate in Hauptrichtung 2 (ksi)
    omega = 0.0;
    Lx = 0; // Lagerbedingung  bei Eingabe: 0=frei, 1=fest, später enthält L() die Gleichungsnummern
}
class TElement {
    EModul;
    GModul;
    ...
    R2 = [0.0, 0.0];
    omegaQ = [0.0, 0.0];
    y1; // Koordinaten im Hauptsystem
    y2;
    z1;
    z2;
    omega = [0.0, 0.0];
    u = [0.0, 0.0];
    ry; // Querschnittswerte für Biegedrillknicken
    rz;
    rOmega;
    tau_p0R = [0.0, 0.0, 0.0];
    tau_p0L = [0.0, 0.0, 0.0];
...
const nTabelle = document.getElementById("nodeTable");
...
export function duennQ() {
    ...
    //let truss = []
    let i, neq;
    let j, k, nod1, nod2, lmi, lmj, kn, ieq;
    let y1, y2, z1, z2, ys, zs, dy, dz, t, sl, h, si2, co2;
    let si, co, sico, si0, co0, cc, bb, cr;
    let Hebely, Hebelz;
    let iP2, iM2;
    let fact, A_omegaQ, A_yOmegaQ, A_zOmegaQ, rOmega, dOmega;
    let It_geschlossen, omega_m, area;
    let rt, ry, rz, y_m, z_m;
    let Gesamtflaeche, I11, I22, phi0, Gesamt_Iyy, Gesamt_Izz, Gesamt_Iyz, Gesamt_It, It_offen, Gesamt_ys, Gesamt_zs, yMh, zMh, // Schubmittelpunkt bezogen auf Schwerpunkt im Hauptachsensystem
    yM, zM; // Schubmittelpunkt bezogen auf Schwerpunkt im yz-Koordinatensystem
    let tau_xs1, tau_xs2, tau_xsm, tau_xs_1, tau_xs_0;
    let normalkraft, moment_y, moment_z, moment_1, moment_2;
    let M_omega;
    let Vy, Vz, V1, V2, Mt1, Mt2;
    let yj, zj, nodj;
    let sigma, sigma2, tau_L, tau_R, fyRd;
    let wert;
    let EModul, mue;
    //let nelem: number = 2
    set_myScreen();
    // Schnittgrößen einlesen
    let input = document.getElementById('Vy');
    Vy = Number(testeZahl(input.value));
    input = document.getElementById('Vz');
    Vz = Number(testeZahl(input.value));
    input = document.getElementById('Nx');
    normalkraft = Number(testeZahl(input.value));
    input = document.getElementById('Mxp');
    Mt1 = Number(testeZahl(input.value));
    input = document.getElementById('Mxs');
    Mt2 = Number(testeZahl(input.value));
    input = document.getElementById('Momega');
    M_omega = Number(testeZahl(input.value));
    input = document.getElementById('My');
    moment_y = Number(testeZahl(input.value));
    input = document.getElementById('Mz');
    moment_z = Number(testeZahl(input.value));
    input = document.getElementById('fyRd');
    fyRd = Number(testeZahl(input.value));
    while (node.length > 0)
        node.pop();
    while (truss.length > 0)
        truss.pop();
    for (i = 0; i < nnodes; i++) {
        node.push(new TNode());
    }
    ...
    if (material_equal.checked) {
        ...
        input = document.getElementById('EMod_ref');
        EModul = Number(testeZahl(input.value));
        input = document.getElementById('mue_ref');
    ...
    I11 = cc + cr;
    I22 = cc - cr;
    if ((Math.abs(Gesamt_Iyz) < 1.e-10) && (Math.abs(bb) < 1.e-10))
        phi0 = 0.0; // zum Beispiel ein Quadrat
    else
        phi0 = 0.5 * Math.atan2(-Gesamt_Iyz, bb);
    console.log("I11", I11);
    console.log("I22", I22);
    ...
    for (k = 0; k < neq; k++)
        R[k] = 0.0; // Aufstellen der rechten Seite
    for (k = 0; k < nelem; k++) {
        for (i = 0; i < 2; i++) {
            lmi = truss[k].lm[i];
            if (lmi >= 0) {
...
//# sourceMappingURL=duennQ.js.map