...
export function main_3D() {
    ...
    const canvas = document.getElementById('c3'); //.querySelector('#c3');
...
//--------------------------------------------------------------------------------------------------------
export function draw_elements(y_s, z_s, y_M, z_M, phi) {
    //--------------------------------------------------------------------------------------------------------
    let i, j;
    let y1, y2, x1, x2, xm, ym;
    ...
    if (scene !== null) {
        ...
        */
        if (maxSigma > 0.0000000000001) {
            ...
            let j = 0, nod1, nod2, sigma1, sigma2;
            for (let i = 0; i < nelem; i++) {
                const vertices = new Float32Array(3 * 6);
                nod1 = truss[i].nod[0];
                nod2 = truss[i].nod[1];
        ...
        }
        {
            ...
            for (i = 0; i < nelem; i++) {
                for (j = 0; j < 3; j++) {
                    ...
                    if (Math.abs(tau_j) > maxTau)
                        maxTau = Math.abs(tau_j);
                }
            }
            if (maxTau > 0.0) {
...
//# sourceMappingURL=grafik_3D.js.map